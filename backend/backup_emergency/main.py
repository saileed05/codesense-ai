...
async def visualize_code(request: Request, code_request: CodeRequest):
    """
    NUCLEAR FIX: Completely bypass code_analyzer for sorting algorithms
    Go STRAIGHT to universal_visualizer
    """
    ...
    try:
        # ✅ NUCLEAR FIX: Detect sorting and FORCE universal_visualizer
        code_lower = code_request.code.lower()
        has_loops = 'for' in code_lower and 'range' in code_lower
        has_array = 'arr' in code_lower or 'array' in code_lower or 'nums' in code_lower
        has_swap = 'arr[' in code_request.code and '=' in code_request.code
        
        is_sorting_algorithm = has_loops and has_array and has_swap
        
        if is_sorting_algorithm:
            logger.info("🚨 DETECTED SORTING ALGORITHM - FORCING UNIVERSAL VISUALIZER!")
            logger.info("🚫 Skipping code_analyzer completely!")
            
            # Go straight to universal visualizer
            tracer = UniversalCodeTracer(code_request.code)
            result = tracer.execute(max_steps=200)
            
            if result['success']:
                logger.info(f"✅ UNIVERSAL VISUALIZER SUCCESS: {result['total_steps']} steps generated!")
                return {
                    "success": True,
                    "steps": result['steps'],
                    "total_steps": result['total_steps'],
                    "language": code_request.language,
                    "analyzer": "universal_visualizer (FORCED)"
                }
            else:
                logger.error(f"❌ UNIVERSAL VISUALIZER FAILED: {result.get('error')}")
                logger.error(f"Traceback: {result.get('traceback', 'No traceback')}")
                
                # Return partial results if available
                if result.get('steps'):
                    logger.warning(f"⚠️ Returning {len(result['steps'])} partial steps")
                    return {
                        "success": False,
                        "steps": result['steps'],
                        "total_steps": len(result['steps']),
                        "error": result.get('error', 'Partial execution'),
                        "partial": True,
                        "language": code_request.language,
                        "analyzer": "universal_visualizer (partial)"
                    }
                else:
                    raise HTTPException(
                        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                        detail=f"Universal visualizer failed: {result.get('error', 'Unknown error')}"
                    )
        
        # For non-sorting code, try code_analyzer first
        logger.info("📈 Not a sorting algorithm - trying code_analyzer...")
        
        try:
            static_steps = generate_execution_steps(code_request.code, code_request.language)
            
            if static_steps and len(static_steps) > 5:
                first_step = static_steps[0]
                viz_type = first_step.get('visualization', {}).get('type', 'none')
                
                if viz_type not in ['none', 'error']:
                    logger.info(f"✅ Using code_analyzer ({len(static_steps)} steps)")
                    return {
                        "success": True,
                        "steps": static_steps,
            ...
            logger.info("⚠️ code_analyzer insufficient, falling back to universal_visualizer...")
            
        except Exception as e:
            logger.warning(f"⚠️ code_analyzer failed: {e}")
        
        # Fallback to universal visualizer
        logger.info("🔄 Using universal_visualizer fallback...")
        tracer = UniversalCodeTracer(code_request.code)
        result = tracer.execute(max_steps=200)
        
        if result['success']:
            logger.info(f"✅ Generated {result['total_steps']} steps")
            return {
                "success": True,
                "steps": result['steps'],
                "total_steps": result['total_steps'],
            ...
            logger.warning(f"⚠️ Partial execution: {result.get('error')}")
            return {
                "success": False,
