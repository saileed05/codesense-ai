"""
Enhanced Code Analyzer v2.3 - FIXED: Skip boring size/length variables
This version intelligently skips intermediate variables like 'n = len(arr)'
to keep visualizations focused on the actual data structures.
"""
import ast
import math
    ...
    if isinstance(node, ast.List):
        ...
        if hasattr(node.func, 'id'):
            ...
            if func_name in ['list', 'dict', 'set', 'tuple', 'deque']:
                ...
                return "length_call"  # ✅ NEW: Detect len() calls
    return "unknown"


def is_boring_variable(var_name: str, var_type: str, node) -> bool:
    """
    ✅ NEW FUNCTION: Determine if a variable assignment is "boring"
    and should be skipped in visualization
    """
    # Skip length/size variables
    if var_name in ['n', 'size', 'length', 'sz', 'len']:
        return True
    
    # Skip if it's just calling len()
    if var_type == "length_call":
        return True
    
    # Skip simple counter initialization
    if var_name in ['i', 'j', 'k', 'idx', 'index'] and var_type == "int":
        return True
...
# ============================================================================
# GRAPH UTILITIES (keeping existing implementation)
# ============================================================================
...
def is_adjacency_list(data) -> bool:
    """Check if data is a valid adjacency list"""
...
# ============================================================================
# GRAPH ALGORITHM DETECTION (keeping existing)
# ============================================================================
...
# ============================================================================
# GRAPH TRAVERSAL SIMULATOR (keeping existing)
# ============================================================================

def simulate_graph_traversal(graph_data: Dict, start_node, algorithm: str = "bfs") -> List[Dict]:
    """Universal graph traversal simulator - supports BFS, DFS, and generic graph display"""
...
# ============================================================================
# MAIN EXECUTION STEP GENERATOR - ✅ WITH BORING VARIABLE FILTERING
# ============================================================================

def generate_execution_steps(code: str, language: str = "python") -> List[Dict[str, Any]]:
    """
    Generate step-by-step execution visualization
    ✅ ENHANCED: Now skips boring intermediate variables like n = len(arr)
    """
    ...
    if language.lower() != "python":
        return [{
            ...
            "description": f"⚠️ Currently only Python is supported",
            "visualization": {
                "type": "none",
                "message": f"💡 Tip: Try Python code with arrays, stacks, or graphs!"
            }
        }]
    ...
    try:
        ...
        # ===== PHASE 1: Collect variable assignments WITH metadata =====
        ...
        # ===== PHASE 4: Regular code analysis with filtering =====
            ...
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name):
                        ...
                        # ✅ CRITICAL FIX: Skip boring variables!
                        if is_boring_variable(var_name, var_type, node.value):
                            print(f"⏭️  Skipping boring variable: {var_name} = {actual_value}")
                        ...
                        # List/Array/Stack/Queue visualization
                            ...
                            if is_stack:
                                steps.append({
                                    ...
                                    "line": node.lineno,
                                    "code": line_code,
                                    "description": f"Create stack '{var_name}' (LIFO) with {len(initial_data)} element(s)",
                                    "visualization": {
                                        "type": "stack",
                                        "name": var_name,
                                        "data": initial_data[:],
                        ...
                        # Simple variable (only if not boring)
                        else:
                            steps.append({
                                "step": step_num,
                                "line": node.lineno,
        ...
        if not steps:
            steps.append({
                ...
                "description": "Try code with: arrays, stacks, queues, graphs, or BFS/DFS",
                "visualization": {
                    "type": "none",
...
if __name__ == "__main__":
    print("=" * 80)
    print("TESTING: Skip Boring Variables Feature")
    print("=" * 80)
    
    # Test 1: Bubble Sort (the problem case!)
    print("\n[TEST 1] Bubble Sort - Should skip 'n = len(arr)'...")
    bubble_sort_code = """
...
"""
    ...
    steps = generate_execution_steps(bubble_sort_code, "python")
    ...
    print("\n" + "=" * 80)
    print("✅ EXPECTED: Step 1 should be array creation, Step 2 should NOT be 'n = len(arr)'")
    print("=" * 80)
    
    # Test 2: Array with boring size variable
    print("\n[TEST 2] Array Operations - Skip size variables...")
    array_code = """
nums = [10, 20, 30, 40]
size = len(nums)
length = len(nums)
...
"""
    ...
    steps2 = generate_execution_steps(array_code, "python")
    print(f"✅ Generated {len(steps2)} steps")
    
    for i, step in enumerate(steps2):
        print(f"  Step {i+1}: {step['description']}")
    
    print("\n✅ EXPECTED: Should skip 'size', 'length', and 'n' variables")
    print("=" * 80)